...
def fitness_function(pop, Rep, time_window):
    ...
    for index in range(len(pop)):
        answer = [[], [], []]           #distance, delivery tardiness, manitenance cost
        fitness.append([])
        #print("\n")
        #print( f'pop[{index}] ={pop[index]}')
        if pop[index][3] == 0:          # if gene is not calculated
            for r in range(Rep):        # repetition

                temp_gene = deepcopy(pop[index])
                #print("temp_gene = pop[index]", pop[index])
                ...
                for l in range(0, loop):                #.......Loading_list (Dedication phase)
                    ...
                    for j in range(0, len(temp_gene[1])):       # selecting a vehicle
                        temp_veh = []
                        ...
                        for c in range(0, len(temp_gene[0])):       # selecting an order
                ...
                for step in range(0, len(loading_list)):                            #.........Production phase
                    for veh in range(0, len(loading_list[step])):
                        ...
                        #..................................Production phase.....................................
                        for cust in loading_list[step][veh]:            # calculate production time for each batch
                            if cust != 0:
                                for o1 in range(0, pn):
                                    #products_wn_ordered[o1] += customer.wn[pop[index][0][cust]][o1]
                                    timer_temp[o1] += customer.process_time[cust][o1]
                                    timer_main[o1] += customer.process_time[cust][o1]
                        #print("timer_temp", timer_temp)
                        # ..................................Maintenance phase..........................
                        for t2 in range(pn):
                            #print("production line                                                 =", t2)
                            fail_prob = [0 for i3 in range(0, pn)]                  # system failure probability
                            if timer_main[t2] > pop[index][2][t2]:                  # maintenance cycle
                                #print("maint_cost                       0", maint_cost)
                                #print("timer_temp[t2]                    2", timer_temp[t2])
                                maint_cost += main.cost[t2]
                                #print("maint_cost                       1", maint_cost)
                                timer_temp[t2] += main.duration[t2]
                                timer_main[t2] = 0
                            #print("product ",  t2,"       timer_main[t2]", timer_main[t2])
                            #print("product ",  t2,"       timer_main", timer_main)
                            ...
                            #print("uni", uni)
                            if fail_prob[t2] > uni :
                                #print("faaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaailure")
                                #print("maint_cost                       1")
                    ...
                    for veh in range(0, len(loading_list[step])):
                        ...
                        for cust in range(0, len(loading_list[step][veh])):
                            ...
                            if cust == 0:                   # if current location of vehicle is factory then calculate first customer related costs
                                if dest != 0:
                                    dist_cost += vehicle.Fixed_cost[pop[index][1][veh]] \
                                                 + (customer.distance[0][dest]/100)*vehicle.alpha[pop[index][1][veh]]*cost.fuel*vehicle.fuel_cons[pop[index][1][veh]]
                                    ...
                                    #print("presumed_delivery_time     p0", customer.presumed_delivery_time[pop[index][0][dest]])
                                    #print(" deliver_timer[step][veh][cust]    0", deliver_timer[step][veh][cust])
                                    #print("(deliver_timer[step][veh][cust] - p)", (deliver_timer[step][veh][cust]) - p)
                                ...
                                deliver_timer[step][veh][cust] += load_timer[pop[index][1][veh]]
                                p = customer.presumed_delivery_time[pop[index][0][dest]]
                                #print("presumed_delivery_time      p1", customer.presumed_delivery_time[pop[index][0][dest]])
                                #print(" deliver_timer[step][veh][cust]      1",  deliver_timer[step][veh][cust])
                                #print("( deliver_timer[step][veh][cust] - p)", ( deliver_timer[step][veh][cust] - p))
                                #print("time_window", time_window)
                                if dest != 0:
                                    if time_window < (deliver_timer[step][veh][cust] - p):
        ...
        #print(f'fitness[{index}] = {fitness[index]}')
        #print("...............................................................................................")
...
#        duration = [int(triangular(PMs[scenario][0][0],        # Stochastic
 #                                  PMs[scenario][0][1],
  #                                 PMs[scenario][0][2])),
   #                 int(triangular(PMs[scenario][1][0],
    #                               PMs[scenario][1][1],
     #                              PMs[scenario][1][2])),
      #              int(triangular(PMs[scenario][2][0],
       #                            PMs[scenario][2][1],
        #                           PMs[scenario][2][2]))]
        cost = [10, 8, 9]
...
#        fail_dur = [int(triangular(CMs[scenario][0][0],        # Stochastic
 #                                  CMs[scenario][0][1],
  #                                 CMs[scenario][0][2])),
   #                 int(triangular(CMs[scenario][1][0],
    #                               CMs[scenario][1][1],
     #                              CMs[scenario][1][2])),
      #              int(triangular(CMs[scenario][2][0],
       #                            CMs[scenario][2][1],
        #                           CMs[scenario][2][2]))]
